...
- **[summit_agent](https://github.com/CiMillan/summit_agent)** — a Ruby framework for tool-using LLM agents, built around a bounded ReAct loop. The README's "Design Decisions" section is the more interesting read: why the tool registry is a plugin seam that's still unproven, why the LLM backend abstraction is hypothetical until a real provider is wired in, and what I'd change next.
